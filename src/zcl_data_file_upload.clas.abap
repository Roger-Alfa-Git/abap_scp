...
* Unpack input field values such as tablename, dataoption, etc.
        ...
        DATA(ui_dataref) = /ui2/cl_json=>generate( json
        = ui_data ).
        IF ui_dataref IS BOUND.

          ASSIGN ui_dataref->* TO FIELD-SYMBOL(<ui_dataref>).

          tablename = me->get_input_field_value( name =
          `TABLENAME` dataref = <ui_dataref> ).
          dataoption = me->get_input_field_value( name
          = `DATAOPTION` dataref = <ui_dataref> ).
        ENDIF.
...
* OR
        tablename IS INITIAL.
          ...
          if_web_http_status=>bad_request
